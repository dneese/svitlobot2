...
#include <WiFiClientSecure.h> // Для HTTPS (hc-ping)
#include <EEPROM.h>
#include <ESP8266WebServer.h>
#include <ESP8266HTTPClient.h>
#include <DNSServer.h>
#include <Ticker.h> // Для сторожового таймера
...
void reconnectWiFi() {
  ...
  while (WiFi.status() != WL_CONNECTED && millis() - startTime < 20000) { 
    delay(500);
    Serial.print("."); // Індикатор спроби підключення
  }
  Serial.println();
...
// Генерація HTML для головної сторінки
String generateHTML() {
...
void handleSave() { 
  if (!server.hasArg("ssid") || !server.hasArg("password") || !server.hasArg("channelKey") || !server.hasArg("token") || !server.hasArg("apPassword")) {
  ...
  String channelKey = server.arg("channelKey");
  String token = server.arg("token");
  String apPassword = server.arg("apPassword");

  // Обмеження довжини 
  if (ssid.length() > 32 || password.length() > 32 || channelKey.length() > 32 || apPassword.length() > 32 || token.length() > 200) {
    server.send(400, "text/plain", "Input too long.");
    return;
  }
  
  // Діагностика
  Serial.println(">>> Збереження налаштувань...");
...
void handleWiFiScan() { // Сторінка налаштувань.
  ...
  html += "<option value='[SCANNING...]' disabled>Оновлення мереж...</option>";
  html += "</select>";
  ...
  html += "<input type='password' id='apPassword' name='apPassword' value='" + readStringFromEEPROM(AP_PASSWORD_ADDR) + "' placeholder='Залиште пустим для стандартного'>";
...
void handleHelp() { // Сторінка Довідки
  String html = "<!DOCTYPE html>";
  html += "<html lang='uk'>";
  html += "<head>";
  html += "<meta charset='UTF-8'>";
  ...
  html += "<p>Цей пристрій на базі ESP8266 (Wemos D1 mini) призначений для відстеження наявності світла (або інтернету) та надсилання регулярних сигналів (пінгів) на сервіси:</p>";
  html += "<ul>";
...
void setup() {
  ...
  // Налаштування Сторожового Таймера (WDT) на 30 секунд
  ESP.wdtEnable(30000); 
