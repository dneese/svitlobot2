//svitlobot_esp32_2.9  21.11.2025
#include <WiFi.h>              // Замість ESP8266WiFi.h
#include <WiFiClient.h>
#include <WiFiClientSecure.h>
#include <EEPROM.h>            // Бібліотека EEPROM для ESP32
#include <WebServer.h>         // Замість ESP8266WebServer.h
#include <HTTPClient.h>        // Замість ESP8266HTTPClient.h
#include <DNSServer.h>
// Ticker.h видалено, використовуємо внутрішній WDT ESP32
...
WebServer server(80);         // WebServer для ESP32
DNSServer dnsServer;
...
const unsigned long wifiCheckInterval = 30000; // 30 секунд
...
String readStringFromEEPROM(int addr) {
  ...
  // Додаємо кешування (логіка без змін)
...
// --- 4. ФУНКЦІЇ ЗАВАНТАЖЕННЯ ---

void checkBootCount() {
  // EEPROM для ESP32 імітує EEPROM ESP8266, тому логіка залишається (якщо EEPROM.begin(size) було викликано)
...
bool checkInternetConnection() { 
  // Логіка перевірки Інтернету без змін
  WiFiClient client;
...
void setupWiFi() {
  ...
  // Функція WiFi.begin() працює однаково на обох платах
...
void createAccessPoint() {
  ...
  // Функція WiFi.softAP() працює однаково
...
void reconnectWiFi() {
  // Логіка перепідключення без змін
...
void handleWiFiConnection() { 
  if (WiFi.status() == WL_CONNECTED) {
    unsigned long currentTime = millis();
    // Логіка миттєвого пінгу при старті без змін
...
void makeHTTPRequest() {
  ...
  // Використовуємо стандартний WiFiClient для HTTP
  WiFiClient client;
...
void makeHcPingRequest() {
  ...
  if (token.startsWith("https://")) {
    // HTTPS ЗАПИТ
    // На ESP32 потрібна бібліотека WiFiClientSecure.h
    WiFiClientSecure client;
    client.setInsecure(); // Використовуємо setInsecure() для спрощення
...
String formatUptime(unsigned long milliseconds) {
  // Логіка форматування часу без змін
...
// Генерація HTML для головної сторінки (Dashboard) - без змін
String generateHTML() {
...
// Розумна логіка збереження без змін
void handleSave() { 
  ...
  // Якщо поле AdminPassword пусте, воно зберігає старий пароль, якщо ні - новий
  ...
  writeStringToEEPROM(ADMIN_PASSWORD_ADDR, newAdminPassword); 
...
void handleWiFiScan() { 
  if (!authenticate()) return;
  // HTML-код сторінки налаштувань (без змін)
  String html = "<!DOCTYPE html>";
  html += "<html lang='uk'>";
  html += "<head>";
  html += "<meta charset='UTF-8'>";
...
void handleHelp() { 
  ...
  html += "<p>Цей пристрій на базі ESP32 svitlobot_esp32_2.9.1  24.1.2025 призначений для відстеження наявності світла (або інтернету) та надсилання регулярних сигналів (пінгів) на сервіси:</p>";
  html += "<ul>";
...
void setup() {
  Serial.begin(115200);
  Serial.println("\n--- СТАРТ СВІТЛОБОТ ESP32 ---"); 
  
  // Ініціалізація EEPROM на ESP32
  if (!EEPROM.begin(EEPROM_SIZE)) {
    Serial.println("Помилка ініціалізації EEPROM. Перезавантаження.");
    delay(1000);
    ESP.restart();
  }
  ...
  // Встановлюємо час останнього пінгу для МИТТЄВОГО спрацювання (логіка без змін)
  lastPingTime = millis() - pingInterval; 

  setupWebServer(); 
  setupDNS();       

  // На ESP32 сторожовий таймер (WDT) зазвичай налаштовується автоматично. 
  // Явні виклики Ticker для WDT ESP8266 прибрано.
  ...
  readStringFromEEPROM(ADMIN_PASSWORD_ADDR);

  Serial.println("--------------------------------");
}

void loop() {
  // Викликаємо yield(), щоб не блокувати WDT, хоча FreeRTOS на ESP32 робить це менш критичним
  yield(); 
