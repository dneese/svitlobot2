...
// ========================================
// SVITLOBOT v3.6 
// https://github.com/dneese/svitlobot2
// ========================================
...
void loadConfig() {
  ...
  Serial.printf("[CONFIG] SSID: %s | AP: %s | UPS: %s\n",
                config.ssid.c_str(),
                config.apPassword.length() > 0 ? "PROTECTED" : "OPEN",
...
// ========================================
// МЕРЕЖА - З ПЕРЕВІРКОЮ ІНТЕРНЕТУ
// ========================================

void checkInternetAccess() {
  static unsigned long lastCheck = 0;
  if (millis() - lastCheck < 30000) return; // Кожні 30 сек
  lastCheck = millis();
...
void performPings() {
  ...
  if (!upsProbeOk()) {
    ...
    Serial.println("[PING] Пропущено через UPS (немає світла в квартирі)");
    return;
  }
...
void setupWebServer() {
  server.on("/", HTTP_GET, [](AsyncWebServerRequest *request) {
    String html = F(
      "<!DOCTYPE html><html><head><meta charset='UTF-8'>"
      "<meta name='viewport' content='width=device-width,initial-scale=1'>"
      "<title>Svitlobot v3.2.4</title>%CSS%</head><body>"
  ...
  server.on("/config", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    String html = F(
      ...
      "<input type='text' name='upsip' id='upsip' value='%UPS_IP%' placeholder='IP контрольного пристрою (192.168.1.50)'>"
  ...
  server.on("/save", HTTP_POST, [](AsyncWebServerRequest *request) {
    ...
    // AP password - КРИТИЧНО: треба перезапустити AP!
    if (request->hasParam("clearappass", true)) {
      config.apPassword = "";
      restartAP = true;
      Serial.println("[SAVE] AP пароль ВИДАЛЕНО -> буде ВІДКРИТИЙ");
    ...
    // ⭐ ЗАСТОСУВАННЯ AP ПАРОЛЯ БЕЗ ПОВНОГО ПЕРЕЗАВАНТАЖЕННЯ
    if (restartAP && state.apActive) {
      Serial.println("[SAVE] Перезапуск AP з новим паролем...");
      WiFi.softAPdisconnect(true);
      delay(500);
      
      if (config.apPassword.length() >= 8) {
        WiFi.softAP(AP_SSID, config.apPassword.c_str());
        Serial.printf("[AP] ✅ Перезапущено з паролем: %s\n", config.apPassword.c_str());
      } else {
        WiFi.softAP(AP_SSID, NULL);
        Serial.println("[AP] ⚠️ Перезапущено БЕЗ пароля (ВІДКРИТИЙ)");
      }
    ...
    if (restartWiFi) {
      resp += "<p style='text-align:center;color:#00fafa;font-size:18px'>WiFi буде перепідключено через <span id='c'>3</span> секунди...</p>"
              "<script>let t=3;setInterval(()=>{t--;document.getElementById('c').innerText=t;if(t<=0)location.href='/'},1000)</script>";
    } else if (restartAP) {
      resp += "<p style='text-align:center;color:#ffc107;font-size:16px'>⚠️ AP пароль оновлено!<br>Переподключіться до <b>" + String(AP_SSID) + "</b></p>"
...
void setup() {
  ...
  Serial.println("\n╔════════════════════════════════╗");
  Serial.println("║ SVITLOBOT v3.6 UPS              ║");
  Serial.println("║ github.com/dneese/svitlobot     ║");
  Serial.println("╚════════════════════════════════╝\n");
  ...
  if (!WiFi.isConnected()) {
    if (config.apPassword.length() >= 8) {
      WiFi.softAP(AP_SSID, config.apPassword.c_str());
      Serial.printf("[AP] ✓ %s (🔒 PROTECTED: %s)\n", AP_SSID, config.apPassword.c_str());
    } else {
      WiFi.softAP(AP_SSID, NULL);
      Serial.printf("[AP] ⚠️ %s (OPEN)\n", AP_SSID);
    }
